...
(function() {
    ...
    const createDSPGraph = () => {
        ...
        // 400Hz low-pass if activated, otherwise fully open (20000Hz)
    ...
    // Global Event Handlers for SPA survivability
    ...
    // Run initial sync in case DOM is already mounted
    window.syncFXPlayerUI();

    // Aggressively preload music.mp3 in the background to update total duration automatically
    if (!audioBuffer) {
        // Safe context creation without waiting for gesture, might be suspended, that's fine
