...
function samplePath(d: string, n: number) {
  if (!tempSvg) {
  ...
  pe.setAttribute('d', d);
  tempSvg.appendChild(pe);
  const L = pe.getTotalLength(), pts = [];
  for (let i = 0; i < n; i++) {
    const p = pe.getPointAtLength(L * i / n);
    pts.push({ x: p.x, y: p.y });
  }
  tempSvg.removeChild(pe);
  return pts;
}

function radialNorm(poly: any[], cx = 0, cy = 0) {
  let mx = 0;
...
function buildRoundedSVG(verts: any[]) {
  const M = verts.length;
  if (M < 3) return '';
  const dir: any[] = [], el: any[] = [];
  for (let i = 0; i < M; i++) {
    const a = verts[i], b = verts[(i + 1) % M], dx = b.x - a.x, dy = b.y - a.y, L = Math.hypot(dx, dy);
    dir.push(L > 0 ? { x: dx / L, y: dy / L } : { x: 1, y: 0 });
    el.push(L);
  }
  const beta: any[] = [], des: any[] = [];
  for (let i = 0; i < M; i++) {
  ...
  for (let i = 0; i < M; i++) {
    const s = des[i] + des[(i + 1) % M];
    if (s > el[i] && s > 0) es[i] = el[i] / s;
  }
  const cut: any[] = [];
...
function repeatAround(tmpl: any[], repeat: number, cx: number, cy: number, mirror: boolean) {
...
function customPoly(tmpl: any[], repeat: number, cx: number, cy: number, mirror: boolean) {
  const verts = repeatAround(tmpl, repeat, cx, cy, mirror);
  return radialNorm(samplePath(buildRoundedSVG(verts), N), cx, cy);
}

function starPoly(spokes: number, outerR: number, innerR: number, crn: number, rotRad: number) {
  const verts = [];
  const total = spokes * 2;
...
function regPoly(vertices: number, radius: number, crn: number, rotRad: number) {
  const verts = [];
...
function alignTo(ref: any[], tgt: any[]) {
  const n = Math.min(ref.length, tgt.length);
  if (!n) return tgt;
...
function render(ts: number) {
  ...
  if (polyEl) polyEl.setAttribute('points', pts.trimEnd());

  const rot = (CONST_ROT * (base + rawF) + EXTRA_ROT * sf) % 360;
  if (svgEl) svgEl.style.transform = `rotate(${rot}deg)`;
