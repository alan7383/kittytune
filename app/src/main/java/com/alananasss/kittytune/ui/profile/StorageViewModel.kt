...
class StorageViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun cleanAudio() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (uriStr == null) {
                context.filesDir.listFiles()?.forEach { file ->
                    if (file.name.endsWith(".mp3") || file.name.startsWith("track_")) {
                        file.delete()
                    }
                }
            } else {
                try {
                    ...
                    docFile?.listFiles()?.forEach { file ->
                        val name = file.name ?: ""
                        if (name.endsWith(".mp3") || name.startsWith("track_")) {
                            file.delete()
                        }
                    }
                } catch (e: Exception) { e.printStackTrace() }
    ...
    fun cleanImages() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (uriStr == null) {
                context.filesDir.listFiles()?.forEach { file ->
                    if (file.name.endsWith(".jpg") || file.name.startsWith("art_")) {
                        file.delete()
                    }
                }
            } else {
                try {
                    ...
                    docFile?.listFiles()?.forEach { file ->
                        val name = file.name ?: ""
                        if (name.endsWith(".jpg") || name.startsWith("art_")) {
                            file.delete()
                        }
                    }
                } catch (e: Exception) { e.printStackTrace() }
