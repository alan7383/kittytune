...
object BackupManager {
    ...
    suspend fun createBackup(context: Context, uri: Uri) {
        withContext(Dispatchers.IO) {
            ...
            val stats = db.getEventsAfter(0L)
    ...
    suspend fun restoreBackup(context: Context, uri: Uri) {
        withContext(Dispatchers.IO) {
            ...
            } ?: throw Exception(context.getString(R.string.error_backup_read))
            ...
            data.listeningStats?.forEach { db.insertStatsEvent(it.copy(id = 0)) }
    ...
    private fun restorePrefs(context: Context, prefName: String, map: Map<String, Any?>) {
        ...
        map.forEach { (key, value) ->
            when (value) {
                ...
                is Number -> {
                    when {
                        key.startsWith("time_") || key == "last_position" -> {
                            editor.putLong(key, value.toLong())
                        }
                        key == "font_wdth" || key == "font_slnt" || key == "font_rond" || key == "font_grad" || key == "font_opsz" || key == "lyrics_font_size" -> {
