    ...
    class PlaybackService : MediaSessionService() {
        ...
        // --- Anti-Spam & Image Cache Variables ---
        ...
        private fun handleLikeToggle() {
            ...
            if (LikeRepository.isTrackLiked(track.id)) LikeRepository.removeLike(track.id) else LikeRepository.addLike(track)
        }
        ...
        override fun onCreate() {
            ...
            val sessionIntent = Intent(this, MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_CLEAR_TOP }
            ...
            mediaSession = MediaSession.Builder(this, forwardingPlayer).setId("KittyTuneSession").setSessionActivity(pendingIntent).build()
        ...
        private fun requestUpdate(delayed: Boolean = true, isForegroundServiceStart: Boolean = false) {
            ...
            if (isForegroundServiceStart) {
                updateJob?.cancel()
                performUpdate(forceNotification = true)
                return
            }
        ...
        private fun performUpdate(forceNotification: Boolean = false) {
            ...
            if (!forceNotification && title == lastNotifTitle && artist == lastNotifArtist &&
                ...
                isLiked == lastLikedState && currentSpeed == lastPlaybackSpeed && currentAlbumArt != null) {
                return
            }
            ...
            val duration = currentTrack?.durationMs?.takeIf { it > 0 }
        ...
        private fun buildNotification(title: String, artist: String, bitmap: Bitmap?): Notification {
            val player = MusicManager.player
            val isPlaying = player.isPlaying
            val openAppIntent = Intent(this, MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_CLEAR_TOP }
            val pendingOpenIntent = PendingIntent.getActivity(this, 0, openAppIntent, PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT)
            ...
            val builder = NotificationCompat.Builder(this, CHANNEL_ID)
                ...
                .addAction(NotificationCompat.Action(R.drawable.ic_skip_previous, getString(R.string.desc_previous), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS)))
    
            if (isPlaying) {
                builder.addAction(NotificationCompat.Action(R.drawable.ic_pause, getString(R.string.desc_pause), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_PAUSE)))
            } else {
                builder.addAction(NotificationCompat.Action(R.drawable.ic_play_arrow, getString(R.string.desc_play), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_PLAY)))
            }
    
            builder.addAction(NotificationCompat.Action(R.drawable.ic_skip_next, getString(R.string.desc_next), MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_SKIP_TO_NEXT)))
            builder.setStyle(MediaStyle().setMediaSession(mediaSessionCompat.sessionToken).setShowActionsInCompactView(0, 1, 2).setShowCancelButton(true).setCancelButtonIntent(MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_STOP)))
